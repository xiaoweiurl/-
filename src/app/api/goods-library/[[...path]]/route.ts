...
/**
 * Goods Library API 代理路由
 *
 * 将前端 /api/goods-library/* 请求转发到 Java 后端的 /goods-library/* 路径。
 */
...
async function proxy(request: NextRequest, method: string) {
  ...
  const backendPath = request.nextUrl.pathname.replace('/api/goods-library', '/goods-library');
  ...
  console.log(`[GoodsLibrary Proxy] ${method} ${targetUrl}`);
  ...
  try {
    ...
    console.error('[GoodsLibrary Proxy] error:', error);
