...
/**
 * 钉钉 H5 免登：转发 Java POST /auth/dingtalk，并种 httpOnly session_id Cookie
 * （与 /api/auth/login 相同，因 Java 只回 X-Session-Id）。
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const authCode = typeof body.authCode === 'string' ? body.authCode : body.code;
    if (!authCode || typeof authCode !== 'string' || !authCode.trim()) {
      return NextResponse.json({ success: false, error: '缺少钉钉授权码' }, { status: 400 });
    }

    const goodsId = body.goodsId;
    const response = await backendFetch('/auth/dingtalk', {
      method: 'POST',
      body: { authCode: authCode.trim(), goodsId },
    });
    ...
    if (!response.ok || result.success === false) {
      return NextResponse.json(
        {
          success: false,
          error: result.error || result.message || '钉钉免登失败',
    ...
    console.error('[API] 钉钉免登失败:', error);
    return NextResponse.json(
      { success: false, error: error instanceof Error ? error.message : '钉钉免登失败' },
      { status: 500 },
    );
  }
}
