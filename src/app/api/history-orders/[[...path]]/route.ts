...
/**
 * History Orders API 代理路由
 *
 * 将前端 /api/history-orders/* 请求转发到 Java 后端的 /history-orders/* 路径。
 * 历史订单（已审核投入生产的销售订单）只读展示。
 */
...
async function proxy(request: NextRequest, method: string) {
  ...
  const backendPath = request.nextUrl.pathname.replace('/api/history-orders', '/history-orders');
  ...
  console.log(`[HistoryOrders Proxy] ${method} ${targetUrl}`);
  ...
  try {
    ...
    console.error('[HistoryOrders Proxy] error:', error);
