...
CREATE TABLE IF NOT EXISTS position_knowledge_cards (
    ...
    -- 卡片编号与基础信息
    card_code VARCHAR(50) NOT NULL,              -- 卡片编号: KC-XXXXXXXX
    submit_date VARCHAR(20),                     -- 提交日期
    department VARCHAR(100),                     -- 所属部门

    -- 一、岗位基本信息
    position_name VARCHAR(100) NOT NULL,        -- 岗位名称(必填)
    on_duty_person VARCHAR(100),                 -- 在岗人员
    report_to VARCHAR(100),                      -- 汇报上级
    team VARCHAR(100),                           -- 所属团队: 品牌运营/产品开发/供应链/财务/投资委员会
    position_nature VARCHAR(50),                 -- 岗位性质: 全职/兼职/顾问/Agent辅助

    -- 二、岗位职责
    core_duties TEXT,                            -- 核心职责(不超过5条，JSON数组格式)
    auxiliary_duties TEXT,                       -- 辅助职责(JSON数组格式)

    -- 三、关键产出物
    key_outputs TEXT,                            -- 关键产出物(JSON数组: [{deliverable, frequency, recipient, standard}])
    ...
    -- 系统字段
    user_id VARCHAR(100) NOT NULL,               -- 创建人ID
    company VARCHAR(50) NOT NULL,                -- 所属公司(数据隔离)
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- 索引
CREATE INDEX IF NOT EXISTS idx_pkc_card_code ON position_knowledge_cards(card_code);
...
CREATE INDEX IF NOT EXISTS idx_pkc_embedding_status ON position_knowledge_cards(embedding_status);
