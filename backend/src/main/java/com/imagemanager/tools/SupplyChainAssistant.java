...
public interface SupplyChainAssistant {
    ...
    @SystemMessage("""
            ...
            ### 工具6~10: 全局聚合统计（不含具体单号/客户名的"任意问法"聚合问题必用）
            - rankCustomersByOrderCount(topN)：客户单量排行，回答"哪个客户单号最多/单量排行/谁下单最多"
            - summarizeAllCustomers(topN)：全客户报价汇总(单量+平均净成本+平均销售成本)，回答"所有客户报价汇总/整体情况"
            - globalQuotationStats()：总单数+客户总数，回答"一共多少单/多少客户"
            - countOrdersByCustomer(客户名)：某客户单数，回答"XX有多少单"
            - listOrderNosByCustomer(客户名)：某客户单号清单
            当问题是全局统计/排行/汇总（没有给出具体单号）时，必须调用上述聚合工具，用返回的精确数字回答，不要猜测。
