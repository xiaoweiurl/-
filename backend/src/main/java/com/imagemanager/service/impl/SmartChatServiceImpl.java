...
public class SmartChatServiceImpl implements SmartChatService {
    ...
    private String buildBusinessBaseConstraints() {
        return "\n\n【基础约束（铁律，两模式通用）】" +
                "\n0. 输出格式约束：仅使用标准 Markdown 语法（**加粗**、列表、表格、标题），严禁使用任何 HTML 标签（如 <font>、<span>、<b>、<div>、<br> 等）；强调内容一律用 **加粗**，不要用标签包裹关键词。" +
                ...
                "【销售订单】条目中的品名与业务员为订单关键字段，必须在订单表格中各自单独一列展示；业务员标注为《业务员数据未维护》时如实呈现，并明确提示用户该订单业务员数据没有维护、请补录；" +
                "上下文附带【数据缺失说明】条目时，必须在回答末尾明确列出该货号哪些维度暂无ERP数据，提示用户同步或补录，禁止静默忽略；" +
